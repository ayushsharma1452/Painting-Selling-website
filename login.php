...
if(isset($_POST['submit'])){
	...
	//Inserting values
	$sql="INSERT into users(name,email,contactNo,password,shippingAddress,billingAddress)
			VALUES('$name','$email','$contactNo','$password','$shippingAddress','$billingAddress')";
...
if(isset($_POST['login'])){
	...
	$sql_login="SELECT * from users where email = '$login_email' AND password = '$login_password'";
	if($result=mysqli_query($con,$sql_login)){
		$count=mysqli_num_rows($result);
		if($count>0){
				$row=mysqli_fetch_assoc($result);				
				$_SESSION['welcomeName']=$row['name'];
				$_SESSION['useri']=$row['id'];
				$_SESSION['login']=true;
				header("Location: my-cart.php");
				exit;
			}	
		else{
			echo "<script type='text/javascript'>alert('No Crediantials were found in the database!');</script>";
		}
	}
	else{
